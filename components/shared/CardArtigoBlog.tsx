...
const CardArtigoBlog = ({ artigo }: { artigo: IArtigo }) => {
   return (
      <Link href={`/blog/${artigo.slug}`} className="border transition hover:bg-theme1 hover:text-white hover:outline-2 outline-theme1 relative group">
         <Image
         ...
         <div className="absolute p-2 sm:p-3 top-0 right-0" style={{ background: artigo.categoria.cor }}>
            <Image className="size-9 sm:size-11 invert-100" width={300} height={300} src={`/icons/categorias-blog/${artigo.categoria.slug}.png`} alt="Ilustracao da categoria" />
         </div>
      </Link>
   );
};
